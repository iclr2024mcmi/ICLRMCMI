...
def parse_option():
    ...
    # Drop class in distillation
    parser.add_argument('-dc', '--drop_class', action='store_true', default=False)
    ...
    if opt.drop_class:
        suffix = '_drop_'
        class_id = [str(elem) for elem in opt.drop_class_idx]
        suffix += '_'.join(class_id)

    opt.model_name_save = 'S:{}_T:{}_CMI:{}_{}_r:{}_a:{}_b:{}_trail_{}{}'.format(opt.model_s, opt.model_t, opt.CMIF, opt.distill,
                                                                opt.gamma, opt.alpha, opt.beta, opt.trial, suffix)
